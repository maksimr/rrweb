...
This also can be done by using the `@studio/rrweb-record` package and the CDN service:
...
Besides the `rrweb` and `@studio/rrweb-record` packages, rrweb also provides other packages for different usage.
...
- [@studio/rrweb-all](packages/all): A package that includes `rrweb` and `@studio/rrweb-packer` for easy install.
- [@studio/rrweb-record](packages/record): A package for recording rrweb sessions.
- [@studio/rrweb-replay](packages/replay): A package for replaying rrweb sessions.
- [@studio/rrweb-packer](packages/packer): A package for packing and unpacking rrweb data.
- [@studio/rrweb-types](packages/types): Contains types shared across rrweb packages.
- [@studio/rrweb-utils](packages/utils): Contains utility functions shared across rrweb packages.
...
- [@studio/rrweb-plugin-console-record](packages/plugins/rrweb-plugin-console-record): A plugin for recording console logs.
- [@studio/rrweb-plugin-console-replay](packages/plugins/rrweb-plugin-console-replay): A plugin for replaying console logs.
- [@studio/rrweb-plugin-sequential-id-record](packages/plugins/rrweb-plugin-sequential-id-record): A plugin for recording sequential IDs.
- [@studio/rrweb-plugin-sequential-id-replay](packages/plugins/rrweb-plugin-sequential-id-replay): A plugin for replaying sequential IDs.
- [@studio/rrweb-plugin-canvas-webrtc-record](packages/plugins/rrweb-plugin-canvas-webrtc-record): A plugin for stream `<canvas>` via WebRTC.
- [@studio/rrweb-plugin-canvas-webrtc-replay](packages/plugins/rrweb-plugin-canvas-webrtc-replay): A plugin for playing streamed `<canvas>` via WebRTC.
...
Since rrweb's replayer ([@studio/rrweb-replay](packages/replay/)) only provides a basic UI, you can choose [rrweb-player](packages/rrweb-player/) which is based on rrweb's public APIs but has a feature-rich replayer UI.
